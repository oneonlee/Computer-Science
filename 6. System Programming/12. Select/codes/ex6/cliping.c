...
int main()
{
    ...
    y = read(x, buf, BUFFER_SIZE - 1); // ex5 : read "Chat partner?"
    buf[y] = '\0';
    printf("serv => client : %s\n", buf); // ex5 : print server message ("Chat partner?")
    printf("client => serv : ");
    fgets(buf, BUFFER_SIZE - 1, stdin); // ex5 : enter chat partner
    write(x, buf, strlen(buf) - 1);     // ex5 : send chat partner
